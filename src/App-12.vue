...
onMounted (()=>{
    ...
    // 加载模型
    let mixer = null, actions =  {}, dracoLoader = new DRACOLoader(), loader = new  GLTFLoader(), activeAction = null;
    ...
    function updatePlayer(deltaTime) {
      ...
      // 如果有水平运动，则设置运动的动作
      if(Math.abs(playerVelocity.x)+Math.abs(playerVelocity.z) > 0.1 && Math.abs(playerVelocity.x)+Math.abs(playerVelocity.z) <= 0.2) {
        fadeToAction("Walking")
      }else if(Math.abs(playerVelocity.x)+Math.abs(playerVelocity.z) > 0.3){
